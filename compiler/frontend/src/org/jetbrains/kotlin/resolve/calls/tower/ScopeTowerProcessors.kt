...
internal class ExplicitReceiverScopeTowerProcessor<C>(
    ...
    override fun simpleProcess(data: TowerData): Collection<C> {
        return when (data) {
            ...
            is TowerData.TowerLevel -> if(data.implicitReceiver == null) resolveAsExtension(data.level) else emptyList()
            else -> emptyList()
        }
    }
...
private class NoExplicitReceiverScopeTowerProcessor<C>(
    ...
    override fun simpleProcess(data: TowerData): Collection<C> {
        if (data !is TowerData.TowerLevel) return emptyList()

        val descriptors = data.level.collectCandidates(name)

        return if (data.implicitReceiver == null) {
                descriptors.filter { !it.requiresExtensionReceiver }.map {
