...
internal sealed class TowerData {
    ...
    class NoTowerLevel(val implicitReceiver: ReceiverValue): TowerData()
    class TowerLevel(val level: ScopeTowerLevel, val implicitReceiver: ReceiverValue?) : TowerData()
}
...
class TowerResolver {
    internal fun <C> runResolve(
        ...
        for (implicitReceiver in context.scopeTower.implicitReceivers) {
            collectCandidates(TowerData.NoTowerLevel(implicitReceiver))?.let { return it }
        }

        for (level in context.scopeTower.levels) {
            collectCandidates(TowerData.TowerLevel(level, null))?.let { return it }

            for (implicitReceiver in context.scopeTower.implicitReceivers) {
                collectCandidates(TowerData.TowerLevel(level, implicitReceiver))?.let { return it }
            }
        }
