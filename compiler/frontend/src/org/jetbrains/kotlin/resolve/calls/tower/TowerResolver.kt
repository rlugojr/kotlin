...
internal sealed class TowerData {
    ...
    class TowerLevelWithImplicitReceiver(val level: ScopeTowerLevel, val implicitReceiver: ReceiverValue) : TowerData()
}
...
class TowerResolver {
    internal fun <C> runResolve(
        ...
        for (level in context.scopeTower.levels) {
            ...
            for (implicitReceiver in context.scopeTower.implicitReceivers) {
                collectCandidates(TowerData.TowerLevelWithImplicitReceiver(level, implicitReceiver))?.let { return it }
            }
        }
