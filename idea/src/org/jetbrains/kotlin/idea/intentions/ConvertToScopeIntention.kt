...
abstract class ConvertToScopeIntention(text: String) : SelfTargetingIntention<KtDotQualifiedExpression>(KtDotQualifiedExpression::class.java, text) {
    ...
    private fun findBoundary(receiverExpressionText: String, element: KtDotQualifiedExpression, forward: Boolean): PsiElement {
        ...
        while (true) {
            val sibling =
                    if (forward) targetElement.getPrevSiblingIgnoringWhitespaceAndComments(false)
                    else targetElement.getNextSiblingIgnoringWhitespaceAndComments(false)
            if (sibling is KtDotQualifiedExpression && sibling.isApplicable(receiverExpressionText)) {
